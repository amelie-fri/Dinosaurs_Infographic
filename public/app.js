...
async function dinoProducer() {
  ...
  const obj = {
    speciesArray, factsArray, heightArray, weightArray, dietArray,
  };
  return obj;
}
...
async function createDinos() {
  const miau = await dinoProducer();
  const allthedinos = [];
  // Iterate over the prepared array to create the single Dino objects
  for (let i = 0; i < 7; i++) {
    allthedinos[i] = new Dino(miau.speciesArray[i],
      miau.heightArray[i], miau.weightArray[i], miau.dietArray[i]);
  }
  // Use the information prepared regarding the pigeon to create the pigeon object
  const pigeon = new Dino(miau.speciesArray[7],
    miau.heightArray[7], miau.weightArray[7], miau.dietArray[7]);
...
async function produceArray() {
  ...
  async function whoTaller() {
    ...
    const heightresults = `${counttaller} dinosaurs are taller than you. ${countsmaller} dinosaurs are smaller than you. Equal size: ${countequal} dinosaurs.`;
    // Return the variable
  ...
  async function whoHeavier() {
    ...
    // Store the strings with the results of the comparison in a variable
    const weightresults = `${countheavier} dinosaurs are heavier than you. ${countlighter} dinosaurs are lighter than you. Equal weight: ${countequalweight} dinosaurs.`;
    // Return the variable
  ...
  async function whoDiet() {
    ...
    const dietresult = `${countsamediet} dinosaurs enjoy the same diet as you do.`;
    // Return the variable
    return dietresult;
  }

  const arrayitemfirst = await whoTaller();
  const arrayitemsecond = await whoHeavier();
  const arrayitemthird = await whoDiet();
...
async function createTiles() {
  ...
  const humanname = document.createTextNode(humanData.name);
  // add the name to the tile
  item[4].appendChild(humanname);

  // The Pigeon Tile
  // create text node for the pigeon fact
  const pigeonfactDom = document.createTextNode(pigeonfact);
  ...
  const pigeontext = document.createTextNode(dinoData.pigeon.species);
  // add pigeon to tile
